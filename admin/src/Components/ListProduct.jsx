...
const ListProduct = () => {
  ...
  const fetchInfo = async ()=>{
    await fetch('http://localhost:1023/allproducts').then((res)=>res.json()).then((data)=>{setAllproducts(data)})
  }
  ...
  const remove_product = async (id)=>{
    await fetch('http://localhost:1023/removeproduct',{
      method:'POST',
      headers:{
  ...
  return (
    <div className='p-2 box-border bg-white mb-2 rounded-sm w-full mt-5 lg:ml-5'>
      <h4 className='bold-22 uppercase'>Product List</h4>
      <div className='max-h-[77vh] overflow-auto px-4 text-center '>
        <table className='w-full mx-auto'>
          <thead>
            <tr className='bg-primary bold-10 sm:regular-22 text-start py-12'>
