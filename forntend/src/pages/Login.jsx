...
const Login = () => {
    ...
    const login = async ()=> {
      ...
      let responseData;
      await fetch('http://localhost:1023/login',{
        method:'POST',
        headers:{
    ...
    const signup = async ()=> {
      ...
      let responseData;
      await fetch('http://localhost:1023/signup',{
        method:'POST',
        headers:{
          Accept:'application/formData',
          'Content-Type':'application/json',
      ...
      <div className='w-full max-w-[666px] h-[600px] bg-primary m-auto px-14 py-10 rounded-md'>
        ...
        <div className='flex flex-col gap-4 mt-7'>
          ...
          <input name='email' type='email' value={formData.email} onChange={changeHandler}
           placeholder='Your Email' className='h-8 w-full pl-5 bg-white outline-none rounded-xl text-sm' />
          <input name='password' type='password' value={formData.password} onChange={changeHandler}
      placeholder='Your Password' className='h-8 w-full pl-5 bg-white outline-none rounded-xl text-sm' />
        </div>
